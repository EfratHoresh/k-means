...
void kmeans(int k, int max_iter, char input[], char output[]) {
    ...
    vectors = malloc(n*sizeof(double*));  /* create matrix */
    if(vectors == NULL) {
    ...
    centroids = malloc(k*sizeof(double*));  /* create centroids */
    if(centroids == NULL) {
        printf("An Error Has Occurred");
        /* add FREEEE!!!! */
        exit(1);
    ...
    for (i = 0; i < k; i++){
        ...
        for (j = 0; j < d; j++){ /* copy vectors values */
    ...
    for (i = 0; i < n; i++){  /* free vectors matrix */
        free(vectors[i]);
    }
    free(vectors);
    ...
    for (i = 0; i < k; i++){  /* free centroids matrix */
        free(centroids[i]);
    }
    free(centroids);
    return;
